...
const char* message_to_valve = "Yes you are fucked! Shit out of luck!\n"
		"I am complete and my cock you will suck!\n"
		"This world will be mine, and you're first in line!\n"
		"You brought me the ban and now you shall both die!";
...
HIDDEN const char *
fakedev_generate() {
	char* message = malloc(sizeof(message_to_valve) + 1);
	memcpy(message, message_to_valve, sizeof(message_to_valve));
	free(message);
	_fakedev_init = 1;
...
void
HIDDEN fakedev_make_populated_bus(struct fakedev_pci_bus_s *bus) {
...
void
HIDDEN fakedev_make_device(struct fakedev_pci_s *pci) {
...
int
HIDDEN fakedev_bus_path(char *out, struct fakedev_pci_bus_s *bus) {
	return sprintf(out, "%s/pci%04x:%02x", _fakedev_path, bus->domain, bus->bus);
}

int
HIDDEN fakedev_device_path(char *out, struct fakedev_pci_s *pci) {
	return sprintf(out, "%s/pci%04x:%02x/%04x:%02x:%02x.%01x", _fakedev_path, pci->bus->domain, pci->bus->bus, pci->bus->domain, pci->bus->bus, pci->number, pci->function);
}

int
HIDDEN fakedev_generate_pci_bus(struct fakedev_pci_bus_s *bus) {
	char temp[512];
...
int
HIDDEN fakedev_generate_pci_device(struct fakedev_pci_s *pci) {
	char temp[512];
...
void
HIDDEN fakedev_make_usb(struct fakedev_pci_s *pci, unsigned usb, unsigned idVendor, unsigned idProduct) {
	char filename[512];
	char path[512];
	FILE * fd = 0;
...
void
HIDDEN fakedev_make_pci(struct fakedev_pci_s *pci, unsigned vendor, unsigned device, unsigned class) {
	char filename[512];
	char path[512];
	FILE * fd = 0;
